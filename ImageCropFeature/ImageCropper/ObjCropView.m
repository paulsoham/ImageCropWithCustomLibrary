...
#import "ObjCropView.h"
#import "ObjCropOverlayView.h"
#import "ObjCropScrollView.h"

#define ObjCropView_BACKGROUND_COLOR [UIColor colorWithWhite:0.12f alpha:1.0f]

static const CGFloat kObjCropViewPadding = 14.0f;
static const NSTimeInterval kTOCropTimerDuration = 0.8f;
static const CGFloat kObjCropViewMinimumBoxSize = 42.0f;
...
typedef NS_ENUM(NSInteger, ObjCropViewOverlayEdge) {
    ObjCropViewOverlayEdgeNone,
    ObjCropViewOverlayEdgeTopLeft,
    ObjCropViewOverlayEdgeTop,
    ObjCropViewOverlayEdgeTopRight,
    ObjCropViewOverlayEdgeRight,
    ObjCropViewOverlayEdgeBottomRight,
    ObjCropViewOverlayEdgeBottom,
    ObjCropViewOverlayEdgeBottomLeft,
    ObjCropViewOverlayEdgeLeft
};

@interface ObjCropView () <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) ObjCropScrollView *scrollView;         /* The scroll view in charge of panning/zooming the image. */
...
@property (nonatomic, strong, readwrite) ObjCropOverlayView *gridOverlayView;   /* A grid view overlaid on top of the foreground image view's container. */
...
@property (nonatomic, assign) ObjCropViewOverlayEdge tappedEdge; /* The edge region that the user tapped on, to resize the cropping region */
...
- (void)setup
{
    ...
    self.backgroundColor = ObjCropView_BACKGROUND_COLOR;
    ...
    self.cropViewPadding = kObjCropViewPadding;
    ...
    self.scrollView = [[ObjCropScrollView alloc] initWithFrame:self.bounds];
    ...
    self.gridOverlayView = [[ObjCropOverlayView alloc] initWithFrame:self.foregroundContainerView.frame];
...
- (void)updateCropBoxFrameWithGesturePoint:(CGPoint)point
{
    ...
    switch (self.tappedEdge) {
        case ObjCropViewOverlayEdgeLeft:
            ...
            break;
        case ObjCropViewOverlayEdgeRight:
            if (self.aspectRatioLockEnabled) {
                aspectHorizontal = YES;
            ...
            break;
        case ObjCropViewOverlayEdgeBottom:
            if (self.aspectRatioLockEnabled) {
                aspectVertical = YES;
            ...
            break;
        case ObjCropViewOverlayEdgeTop:
            if (self.aspectRatioLockEnabled) {
                aspectVertical = YES;
                yDelta = MAX(0,yDelta);
            ...
            break;
        case ObjCropViewOverlayEdgeTopLeft:
            if (self.aspectRatioLockEnabled) {
                xDelta = MAX(xDelta, 0);
                yDelta = MAX(yDelta, 0);
            ...
            break;
        case ObjCropViewOverlayEdgeTopRight:
            if (self.aspectRatioLockEnabled) {
                xDelta = MIN(xDelta, 0);
                yDelta = MAX(yDelta, 0);
            ...
            break;
        case ObjCropViewOverlayEdgeBottomLeft:
            if (self.aspectRatioLockEnabled) {
                CGPoint distance;
                distance.x = 1.0f - (xDelta / CGRectGetWidth(originFrame));
                distance.y = 1.0f - (-yDelta / CGRectGetHeight(originFrame));
            ...
            break;
        case ObjCropViewOverlayEdgeBottomRight:
            if (self.aspectRatioLockEnabled) {
            ...
            break;
        case ObjCropViewOverlayEdgeNone: break;
    }
    
    //The absolute max/min size the box may be in the bounds of the crop view
    CGSize minSize = (CGSize){kObjCropViewMinimumBoxSize, kObjCropViewMinimumBoxSize};
    ...
    if (self.aspectRatioLockEnabled && aspectHorizontal) {
        ...
        minSize.width = kObjCropViewMinimumBoxSize * aspectRatio;
    }
        
    if (self.aspectRatioLockEnabled && aspectVertical) {
        maxSize.width = contentFrame.size.height * aspectRatio;
        minSize.height = kObjCropViewMinimumBoxSize / aspectRatio;
    }
...
- (ObjCropViewOverlayEdge)cropEdgeForPoint:(CGPoint)point
{
    CGRect frame = self.cropBoxFrame;
    ...
    if (CGRectContainsPoint(bottomLeftRect, point))
        return ObjCropViewOverlayEdgeBottomLeft;
...
- (void)setCropBoxFrame:(CGRect)cropBoxFrame
{
    ...
    cropBoxFrame.size.width  = MAX(cropBoxFrame.size.width, kObjCropViewMinimumBoxSize);
    cropBoxFrame.size.height = MAX(cropBoxFrame.size.height, kObjCropViewMinimumBoxSize);
    ...
    if (self.croppingStyle == ObjCropViewCroppingStyleCircular) {
