//
// ObjCropViewController.m
//
// Soham Paul
//

#import "ObjCropViewController.h"

#import "ObjCropViewControllerTransitioning.h"
#import "ObjActivityCroppedImageProvider.h"
#import "UIImage+CropRotate.h"
#import "ObjCroppedImageAttributes.h"

static const CGFloat kObjCropViewControllerTitleTopPadding = 14.0f;
static const CGFloat kObjCropViewControllerToolbarHeight = 44.0f;

@interface ObjCropViewController () <UIActionSheetDelegate, UIViewControllerTransitioningDelegate, ObjCropViewDelegate>

/* The target image */
@property (nonatomic, readwrite) UIImage *image;

/* The cropping style of the crop view */
@property (nonatomic, assign, readwrite) ObjCropViewCroppingStyle croppingStyle;

/* Views */
@property (nonatomic, strong) ObjCropToolbar *toolbar;
@property (nonatomic, strong, readwrite) ObjCropView *cropView;
...
@property (nonatomic, strong) ObjCropViewControllerTransitioning *transitionController;
...
@implementation ObjCropViewController

- (instancetype)initWithCroppingStyle:(ObjCropViewCroppingStyle)style image:(UIImage *)image
{
    ...
    if (self) {
        ...
        _transitionController = [[ObjCropViewControllerTransitioning alloc] init];

        // Default initial behaviour
        _aspectRatioPreset = ObjCropViewControllerAspectRatioPresetOriginal;

        #if TARGET_OS_MACCATALYST
        _toolbarPosition = ObjCropViewControllerToolbarPositionTop;
        #else
        _toolbarPosition = ObjCropViewControllerToolbarPositionBottom;
        #endif
    }
...
{
    return [self initWithCroppingStyle:ObjCropViewCroppingStyleDefault image:image];
}
...
- (void)viewDidLoad
{
    ...
    BOOL circularMode = (self.croppingStyle == ObjCropViewCroppingStyleCircular);
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.aspectRatioPreset != ObjCropViewControllerAspectRatioPresetOriginal) {
...
- (CGRect)frameForToolbarWithVerticalLayout:(BOOL)verticalLayout
{
    ...
    if (!verticalLayout) { // In landscape laying out toolbar to the left
        ...
        frame.origin.y = 0.0f;
        frame.size.width = kObjCropViewControllerToolbarHeight;
        frame.size.height = CGRectGetHeight(self.view.frame);
    }
    else {
        frame.origin.x = 0.0f;
        frame.size.width = CGRectGetWidth(self.view.bounds);
        frame.size.height = kObjCropViewControllerToolbarHeight;

        if (self.toolbarPosition == ObjCropViewControllerToolbarPositionBottom) {
...
- (CGRect)frameForCropViewWithVerticalLayout:(BOOL)verticalLayout
{
    ...
    if (!verticalLayout) {
        frame.origin.x = kObjCropViewControllerToolbarHeight + insets.left;
        ...
        if (self.toolbarPosition == ObjCropViewControllerToolbarPositionBottom) {
            frame.size.height -= (insets.bottom + kObjCropViewControllerToolbarHeight);
        } else if (self.toolbarPosition == ObjCropViewControllerToolbarPositionTop) {
            frame.origin.y = kObjCropViewControllerToolbarHeight + insets.top;
...
- (CGRect)frameForTitleLabelWithSize:(CGSize)size verticalLayout:(BOOL)verticalLayout
{
    ...
    if (!verticalLayout) {
        x = kObjCropViewControllerTitleTopPadding;
    ...
    if (@available(iOS 11.0, *)) {
        frame.origin.y = self.view.safeAreaInsets.top + kObjCropViewControllerTitleTopPadding;
    }
    else {
        frame.origin.y = self.statusBarHeight + kObjCropViewControllerTitleTopPadding;
    }
...
- (void)adjustCropViewInsets
{
    ...
    if (!self.titleLabel.text.length) {
        if (self.verticalLayout) {
          if (self.toolbarPosition == ObjCropViewControllerToolbarPositionTop) {
    ...
    verticalInset += kObjCropViewControllerTitleTopPadding;
...
- (void)adjustToolbarInsets
{
    ...
    if (@available(iOS 11.0, *)) {
        ...
        if (!self.statusBarHidden && self.toolbarPosition == ObjCropViewControllerToolbarPositionTop) {
...
- (void)showAspectRatioDialog
{
    ...
    NSString *cancelButtonTitle = NSLocalizedStringFromTableInBundle(@"Cancel", @"ObjCropViewControllerLocalizable", resourceBundle, nil);
    NSString *originalButtonTitle = NSLocalizedStringFromTableInBundle(@"Original", @"ObjCropViewControllerLocalizable", resourceBundle, nil);
    NSString *squareButtonTitle = NSLocalizedStringFromTableInBundle(@"Square", @"ObjCropViewControllerLocalizable", resourceBundle, nil);
    
    //Prepare the list that will be fed to the alert view/controller
    
    // Ratio titles according to the order of enum ObjCropViewControllerAspectRatioPreset
    ...
    if (self.allowedAspectRatios == nil) {
        for (NSInteger i = 0; i < ObjCropViewControllerAspectRatioPresetCustom; i++) {
        ...
        for (NSNumber *allowedRatio in self.allowedAspectRatios) {
            ObjCropViewControllerAspectRatioPreset ratio = allowedRatio.integerValue;
    ...
    if (self.customAspectRatioName.length > 0 && !CGSizeEqualToSize(CGSizeZero, self.customAspectRatio)) {
        ...
        [ratioValues addObject:@(ObjCropViewControllerAspectRatioPresetCustom)];
    }
...
- (void)setAspectRatioPreset:(ObjCropViewControllerAspectRatioPreset)aspectRatioPreset animated:(BOOL)animated
{
    ...
    switch (aspectRatioPreset) {
        case ObjCropViewControllerAspectRatioPresetOriginal:
            aspectRatio = CGSizeZero;
            break;
        case ObjCropViewControllerAspectRatioPresetSquare:
            aspectRatio = CGSizeMake(1.0f, 1.0f);
            break;
        case ObjCropViewControllerAspectRatioPreset3x2:
            aspectRatio = CGSizeMake(3.0f, 2.0f);
            break;
        case ObjCropViewControllerAspectRatioPreset5x3:
            aspectRatio = CGSizeMake(5.0f, 3.0f);
            break;
        case ObjCropViewControllerAspectRatioPreset4x3:
            aspectRatio = CGSizeMake(4.0f, 3.0f);
            break;
        case ObjCropViewControllerAspectRatioPreset5x4:
            aspectRatio = CGSizeMake(5.0f, 4.0f);
            break;
        case ObjCropViewControllerAspectRatioPreset7x5:
            aspectRatio = CGSizeMake(7.0f, 5.0f);
            break;
        case ObjCropViewControllerAspectRatioPreset16x9:
            aspectRatio = CGSizeMake(16.0f, 9.0f);
            break;
        case ObjCropViewControllerAspectRatioPresetCustom:
...
- (void)cropViewDidBecomeResettable:(ObjCropView *)cropView
{
    self.toolbar.resetButtonEnabled = YES;
}

- (void)cropViewDidBecomeNonResettable:(ObjCropView *)cropView
{
...
- (id <UIViewControllerAnimatedTransitioning>)animationControllerForPresentedController:(UIViewController *)presented presentingController:(UIViewController *)presenting sourceController:(UIViewController *)source
{
    ...
    self.transitionController.prepareForTransitionHandler = ^{
        typeof (self) strongSelf = weakSelf;
        ObjCropViewControllerTransitioning *transitioning = strongSelf.transitionController;
...
- (void)cancelButtonTapped
{
    ...
    NSString *yesButtonTitle = NSLocalizedStringFromTableInBundle(@"Delete Changes", @"ObjCropViewControllerLocalizable", resourceBundle, nil);
    NSString *noButtonTitle = NSLocalizedStringFromTableInBundle(@"Cancel", @"ObjCropViewControllerLocalizable", resourceBundle, nil);
...
- (void)doneButtonTapped
{
    ...
    if (self.showActivitySheetOnDone) {
        ObjActivityCroppedImageProvider *imageItem = [[ObjActivityCroppedImageProvider alloc] initWithImage:self.image cropFrame:cropFrame angle:angle circular:(self.croppingStyle == ObjCropViewCroppingStyleCircular)];
    ...
    if (self.croppingStyle == ObjCropViewCroppingStyleCircular && (isCircularImageDelegateAvailable || isCircularImageCallbackAvailable)) {
...
- (ObjCropView *)cropView
{
    ...
    if (!_cropView) {
        _cropView = [[ObjCropView alloc] initWithCroppingStyle:self.croppingStyle image:self.image];
...
- (ObjCropToolbar *)toolbar
{
    if (!_toolbar) {
        _toolbar = [[ObjCropToolbar alloc] initWithFrame:CGRectZero];
...
- (void)setCustomAspectRatio:(CGSize)customAspectRatio
{
    ...
    [self setAspectRatioPreset:ObjCropViewControllerAspectRatioPresetCustom animated:NO];
}
